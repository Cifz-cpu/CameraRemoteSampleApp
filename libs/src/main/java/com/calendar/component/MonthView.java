...
public abstract class MonthView extends View {
    ...
    protected int getMonthRowNumber(int year, int month) {
        ...
        int weekNumber = DateUtils.getFirstDayWeek(year, month);
        return (monthDays + weekNumber - 1) % 7 == 0 ? (monthDays + weekNumber - 1) / 7
                : (monthDays + weekNumber - 1) / 7 + 1;
    }
    ...
    protected String iscalendarInfoDes1(int day) {
        if (calendarInfos == null || calendarInfos.size() == 0)
            return "";
        for (CalendarInfo calendarInfo : calendarInfos) {
            if (calendarInfo.day == day && calendarInfo.month == selMonth + 1 && calendarInfo.year == selYear) {
                return calendarInfo.des1;
            }
        }
        return "";
    }
