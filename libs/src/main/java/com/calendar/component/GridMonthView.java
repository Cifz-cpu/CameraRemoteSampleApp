...
import com.calendar.theme.DefaultDayTheme;
...
public class GridMonthView extends MonthView {
    ...
    protected void drawDecor(Canvas canvas, int column, int row, int day) {
        if (calendarInfos != null && calendarInfos.size() > 0) {
            String calenderInfo = iscalendarInfo(day);
            if (TextUtils.isEmpty(calenderInfo) || !TextUtils.equals("Y", calenderInfo))
                return;
...
//         居中显示文字
        float startY = rowSize * row + rowSize / 2 - (paint.ascent() +
                paint.descent()) / 2;
        paint.setStyle(Paint.Style.STROKE);
        String des = iscalendarInfo(day);
        String des1 = iscalendarInfoDes1(day);
        if (day == selDay) {// 日期为选中的日期
            if (!TextUtils.isEmpty(des)) {// desc不为空的时候
                ...
                canvas.drawText(des1, priceX, priceY + 30, paint);
