...
/**
 *
 * Created by Donlann on 2018/4/27.
 */
...
public class CRC16 {
    ...
    public static byte[] getSendBuf(byte[] originalata) {
        byte[] newData = new byte[originalata.length+2];
        System.arraycopy(originalata,0,newData,0,originalata.length);
        CRC16 crc16 = new CRC16();
        crc16.update(originalata, originalata.length);
        int ri = crc16.getValue();
        newData[newData.length - 1] = (byte) (0xff & ri);
        newData[newData.length - 2] = (byte) ((0xff00 & ri) >> 8);
        return newData ;
    }
